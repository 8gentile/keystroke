document.addEventListener("DOMContentLoaded", () => {
  ...
  const gameOver = () => {
    ...
    window.addEventListener("keypress", restartKeystroke, false);
    ...
    ctx.fillText(`${playerName}: ${score}`, canvas.width*0.15, canvas.height*0.4);
    ...
    ctx.fillText("(or press 'r' to play again)", canvas.width*0.15, canvas.height*0.75);
  }

  const restartKeystroke = (e) => {
    e.preventDefault();
  ...
  const collision = () => {
    ...
    if (x > hole.x && x < (hole.x + hole.xw)) {
      ...
      gradient.addColorStop("1.0","red");
      ctx.fillStyle=gradient;
      ctx.fillText(winText, canvas.width*0.35, canvas.height*0.3);
  ...
  const drawTerrain = () => {
    ...
    let grd=ctx.createLinearGradient(water.x + water.xw/2, water.y, water.x + water.xw/2, water.y + water.yw);
  ...
  scoresTable.orderByChild("score").limitToLast(200).on('value', (snapshot, highscores) => {
