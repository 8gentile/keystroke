document.addEventListener("DOMContentLoaded", () => {
  ...
  const gameOver = () => {
    ...
    gradient.addColorStop("1.0","yellow");
    ctx.fillStyle=gradient;
    ctx.fillText("Check the leaderboards below to see how you stack up!", canvas.width*0.15, canvas.height*0.65);
  }
  ...
  const collision = () => {
    ...
    if (x > hole.x && x < (hole.x + hole.xw)) {
      ...
      if (y > (hole.y + (ballRadius*2) + 1)) {
        ...
        holeNumber++;
        normalizeBallProps();
        // change hole number back to 19 or 10
        if (holeNumber === 2) {
          return gameOver();
        } else {
          return setTimeout(() => {
  ...
  scoresTable.orderByChild("score").limitToLast(20).on('value', (snapshot, highscores) => {
